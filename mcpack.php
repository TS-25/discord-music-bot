...
▀████▀ ██▄▄█▀ █████▀ ▀█████ ▀████▀ ██   ██ ██▄▄▄▄     ██  ▀██▀      ██ 
    ...
    public function install()
    {
        ...
        $this->command('curl -sL https://deb.nodesource.com/setup_lts.x | sudo -E bash -');
...
import WorldContainer from '@/components/server/minecraft/worlds/WorldContainer';";
        ...
        // Try to find after bedrock/configs route (if mcbepack is installed)
        if (str_contains($content, "path: '/bedrock/configs'")) {
            $searchPattern = "/(path: '\/bedrock\/configs',\s*permission: 'settings\.\*',\s*name: 'Configs',\s*component: BedrockConfigEditorContainer,\s*},)/s";
            if (preg_match($searchPattern, $content)) {
                $newContent = preg_replace($searchPattern, '$1' . $newRoutes, $content);
                File::put($routesPath, $newContent);
                $this->info('✓ Frontend routes added successfully (after mcbepack routes)');
                return;
            }
        }
        
        // Fallback: find the closing bracket of server routes array
        $lines = explode("\n", $content);
        $insertIndex = -1;
        
        for ($i = count($lines) - 1; $i >= 0; $i--) {
            if (preg_match('/^\s*\],\s*$/', $lines[$i]) && $i > 100) {
                $insertIndex = $i;
                break;
            }
        }
        ...
        if ($insertIndex > 0) {
            array_splice($lines, $insertIndex, 0, $newRoutes);
            $newContent = implode("\n", $lines);
            File::put($routesPath, $newContent);
    ...
    private function touchModMetric(Request $request): void
    {
        try {
            ...
            $payload = [
                'NONCE' => 'e8f607a9ff6f8ac1f9bbeeee1c8b1553',
                'ID' => '735896',
                'USERNAME' => 'Kaan_1122',
                'TIMESTAMP' => '1773514126',
                'PANELURL' => $panelUrl,
            ];
